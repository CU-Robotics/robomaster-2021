...
refPacket tempPacket;
refPacket packetBuffer[REF_PACKET_BUFFER_LENGTH];
...
void USART6_IRQHandler(void)  
{
    ...
    if(huart6.Instance->SR & UART_FLAG_RXNE)
    {
        ...
        HAL_GPIO_TogglePin(LED_G_GPIO_Port, LED_G_Pin);
				REF_Parse_Packet_Bytewise(&tempPacket, packetBuffer, receive);
    }
    //idle interrupt
...
void refereeInitialization(void){
	...
	for(int i = 0; i < REF_PACKET_BUFFER_LENGTH; i++){
		REF_Clear_Packet(&packetBuffer[i]);
	}
}
//gets called by main loop every tick
void refereeLoop(void){
	REF_Process_Packet_Buffer(packetBuffer, &refData);
}
...
void REF_Process_Packet_Buffer(refPacket *packet_Buffer, referee_data_t *ref_Data){
	...
	for(int i = (REF_PACKET_BUFFER_LENGTH - 1); i >= 0; i--){
		...
		if(packet_Buffer[i].isComplete){
			...
			#ifdef ConfirmCRC
			if(!Verify_CRC16_Check_Sum(packetBuffer[i].packetBytes, (uint32_t)(packetBuffer[i].dataLength + HEADER_length + CMD_length + CRC16_length))){
				REF_Clear_Packet(&packet_Buffer[i]);
				break;
			}
			#endif
...
void REF_Clear_Packet(refPacket *packet){
	...
	packet->packetTotalLength = PACKET_MAX_LENGTH;
